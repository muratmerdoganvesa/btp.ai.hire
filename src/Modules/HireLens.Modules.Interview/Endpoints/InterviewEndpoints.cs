...
public static class InterviewEndpoints
{
    public static IEndpointRouteBuilder MapInterviewEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/api/interviews", async (
            IInterviewService interviews,
            CancellationToken ct) => HttpResults.From(await interviews.ListBoardAsync(ct)))
            .WithTags("Interview")
            .RequireAuthorization();

        endpoints.MapGet("/api/interviews/{sessionId:guid}", async (
            Guid sessionId,
            IInterviewService interviews,
            CancellationToken ct) => HttpResults.From(await interviews.GetByIdAsync(sessionId, ct)))
            .WithTags("Interview")
            .RequireAuthorization();

        endpoints.MapPost("/api/interviews/{sessionId:guid}/evaluate", async (
            Guid sessionId,
            IInterviewService interviews,
            CancellationToken ct) => HttpResults.From(await interviews.EvaluateByIdAsync(sessionId, ct)))
            .WithTags("Interview")
            .RequireAuthorization();
        ...
        endpoints.MapPost("/api/candidates/{candidateId:guid}/interview/evaluate", async (
            Guid candidateId,
            IInterviewService interviews,
            CancellationToken ct) => HttpResults.From(await interviews.EvaluateForCandidateAsync(candidateId, ct)))
            .WithTags("Interview")
            .RequireAuthorization();
